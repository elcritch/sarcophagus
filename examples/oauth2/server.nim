...
when isMainModule:
  ...
  router.get("/api/read", withOAuth2BearerAuth(readHandler, config, @["sync:read"]))
  router.get("/api/write", withOAuth2BearerAuth(writeHandler, config, @["sync:write"]))
  router.get("/api/whoami", withOAuth2BearerAuth(claimsHandler, config, @["sync:read"]))
  router.get("/api/admin", withOAuth2BearerAuth(readHandler, config, @["sync:admin"]))
